...
import { generateMockStory, type MockStoryDraft, type MockStorySettings } from "@/lib/generate/mock-story";
...
export interface RunStoryGenerationInput {
  ...
  mode?: GenerationMode;
  /** A story the caller already built (Manual Content: the client's own
...
export async function runStoryGeneration(
  ...
  // would misrepresent a $0 draft as a real generation. A prebuilt (manual)
  // draft writes no LLM at all, so the gate doesn't apply.
  const isManual = Boolean(input.prebuiltDraft);
  if (!isManual && mode === "live" && !resolved.hasCredential) {
  ...
  const draft: MockStoryDraft = input.prebuiltDraft
    ? input.prebuiltDraft
    : mode === "live"
  ...
  await notify({
    ...
    category: "review",
    title: isManual
      ? "Your script is ready for review"
      : mode === "live"
        ? "New AI story generated"
        : "New draft story generated",
    body: isManual
      ? `"${draft.topic}" was built from your own script and is ready to review in the pipeline.`
      : mode === "live"
