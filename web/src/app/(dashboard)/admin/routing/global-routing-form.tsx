...
import { updateGlobalRoutingAction } from "./actions";
...
export function GlobalRoutingForm({ value }: { value: ModelRoutingValue }) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await updateGlobalRoutingAction(formData);
      if (!result.ok) {
        setError(result.error ?? "Couldn't save global routing.");
        return;
      }
      setSaved(true);
    });
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <Field id="image_high" name="image_high" label="High" defaultValue={value.image.High} disabled={isPending} />
          <Field id="image_medium" name="image_medium" label="Medium" defaultValue={value.image.Medium} disabled={isPending} />
          <Field id="image_low" name="image_low" label="Low" defaultValue={value.image.Low} disabled={isPending} />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 border-t border-border pt-6">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <Field id="motion_premium" name="motion_premium" label="Premium" defaultValue={value.motion.premium} disabled={isPending} />
          <Field id="motion_standard" name="motion_standard" label="Standard" defaultValue={value.motion.standard} disabled={isPending} />
          <Field id="motion_cheap" name="motion_cheap" label="Cheap" defaultValue={value.motion.cheap} disabled={isPending} />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 border-t border-border pt-6">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <Field id="thumbnail" name="thumbnail" label="Thumbnail" defaultValue={value.thumbnail} disabled={isPending} />
        </div>
      </div>
      ...
      {saved && !error ? (
        <div className="flex items-start gap-2 rounded-lg border border-[var(--status-approved)]/30 bg-[var(--status-approved)]/10 px-3 py-2 text-xs text-[var(--status-approved)]">
          ...
          <span>Global model routing saved.</span>
        </div>
      ) : null}
      ...
      >
        ...
        {isPending ? "Saving…" : "Save global routing"}
      </button>
    </form>
  );
}
