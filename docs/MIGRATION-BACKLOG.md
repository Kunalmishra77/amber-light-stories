...
| **M11** | Automation Engine — durable workflow/job runtime (Part 5 target) | core reliability/scale; absorbs M4/M5 — **COMPLETE (2026-07-21)**. M11-1 Durable Job Engine core; M11-2 Scheduler→durable jobs; Phase A tenant-fair queue + plan-aware concurrency caps (mig 017); Phase B durable generation/publish/analytics handlers; Phase C workflow/DAG runtime (mig 018); Phase D failure recovery + escalation; Phase E cost governor + circuit breakers; Phase F engine observability + re-drive/cancel ops; Phase G scheduler misfire policy + cadence simulation (mig 019). Deferred by design: visual workflow builder (R1-01), automation marketplace (R1-02), version control (R1-04), execution replay/clone (P5-06 remainder). |
| **M12** | AI Generation Pipeline - content intelligence (Part 6 target) | runs on M11 - **CONSOLIDATED PHASE COMPLETE (2026-07-21)**. G1 Asset & Governance library (mig 020) - G2 Format Profiles + locale + calendar-as-input (mig 021) - G3 Quality + Compliance gates (mig 022) - G4 Content Memory + Decision Records (mig 023) - G5 Pipeline Analytics Center (no new storage) - G6 expanded 29-stage pipeline with explicit gated semantics. EXECUTION-GATED (no mock intelligence built): R1-02 variations, R1-03 RAG, R1-05 trend/competitor, R1-06 thumbnail CTR, R1-08 learning, R1-09/P6-12 translation + localized voice, R1-01 repurposing render. |
| **M13** | Enterprise Security - identity, authN/authZ, Vault, audit, compliance (Part 7 target) | cross-cutting; hardens M1/M2/M8 - **CONSOLIDATED PHASE COMPLETE (2026-07-21)**. S1 Policy & Zero-Trust core (mig 024) - S2 Identity/RBAC/PAM/Organizations, additive (mig 025) - S5 tamper-evident audit foundation (mig 026, closes the active audit_log gap) - S3 sessions/devices/threats/incidents (mig 027) - S4 Vault lifecycle + KMS (mig 028) - S5 compliance/privacy/DLP (mig 029) - S6 break-glass (mig 030). EXECUTION-GATED (never faked): SSO/SCIM (external IdP + Supabase SSO plan), BYOK (external KMS), impossible-travel (IP-geolocation provider). |
...
| ID | Issue / gap | Sev | Task | Status | Source |
|---|---|---|---|---|---|
| ISS-P5-01 | No generic **Workflow/DAG engine** (seq/parallel/conditional/branch/merge/nested/versioned/template) — only a linear pipeline concept | Critical | M11/M4 | **Done (M11 Phase C)** — workflow/DAG runtime (`lib/workflow/`, migration 018: `workflow_runs`/`workflow_steps` + jobs.workflow_run_id/step_id). Sequential, parallel, fan-in, dependency edges, conditional branching (cascading skips resolved to a fixed point), failure propagation, run-level state/context checkpoint, idempotent start + idempotent step launch (`wf:<run>:<step>`), tenant-isolated. Orchestrates durable jobs — it is NOT a second execution engine. Visual builder deliberately not built (runtime first). | P5 §4 |
...
| ISS-P5-03 | No **queue + stateless workers + DLQ** infrastructure | Critical | M11 | **Done (M11-1 + Phase A/F)** — atomic `claim_jobs` (FOR UPDATE SKIP LOCKED) + stateless `processJobs` workers + `/api/cron/process-jobs` + `reap_stale_jobs` crash recovery + dead-letter queue; tenant-fair admission with plan-aware concurrency caps (migration 017); DLQ visible and re-drivable in the ops console. | P5 §8, ADR-031; extends ISS-P2-05 |
...
| ISS-P5-06 | No **execution management** (history/timeline/replay/comparison/export/clone/recovery) | High | M11 | **Partially done (M11 Phase F)** — durable job history, status, attempts, lease state, workflow linkage, error details, and operator re-drive/cancel at `/admin/queue/jobs`. Timeline/replay/comparison/clone/export remain future work. | P5 §8 |
| ISS-P5-07 | No **layered Failure Recovery** (checkpoint/rollback/DLQ/escalation/RCA/failure categories) | High | M11 | **Done (M11 Phase D)** — layered recovery: stale-lease reaping (crash/deploy safe), checkpoint-based resume (`jobs.checkpoint` + workflow run/step state), exponential backoff retry, explicit terminal states, dead-letter queue, non-retryable fast-fail, and escalation of every dead job to `event_log`. Full RCA/failure-category taxonomy remains future work. | P5 §9, ADR-030 |
| ISS-P5-08 | No **engine-level cost governor** (workspace/monthly budgets, parallel cost limits, duplicate detection) | High | M11/M6 | **Done (M11 Phase E)** — engine-level cost governor `checkTenantBudget` enforced BEFORE work starts, summing the existing `api_usage` ledger against the plan's `monthly_cost_usd` (no duplicate cost store). Over-budget jobs fail terminally (never retried). | P5 §10, ADR-032 |
| ISS-P5-09 | No **provider auto-switching / circuit breakers** (cost-bounded fallback) | Medium | M11 | **Done (M11 Phase E)** — provider circuit breakers derived from the existing `provider_health` store (closed/open/half-open with cool-off); the AI Gateway skips open providers without calling them and fails over to the next candidate within the existing selection chain. | P5 §9, ADR-033 |
| ISS-P5-10 | No **tenant-fair queue partitioning + per-plan concurrency caps** (noisy-neighbor protection) | High | M11/M1 | **Done (M11 Phase A)** — `tenant_job_concurrency()` (plan-aware, from `plans.limits.job_concurrency`) + fair admission in `claim_jobs`: per-tenant caps and ordering by fewest-currently-running, so a flooding tenant cannot monopolise or starve others. Verified live. | P5 §12, ADR-031 |
| ISS-P5-11 | No **idempotency + exactly-once side effects** on retries/publishing | High | M11 | **Done (M11, precise semantics)** — EXACTLY-ONCE *enqueue* per (tenant, idempotency_key) via DB unique index; AT-LEAST-ONCE *execution* (a worker may crash after doing work); therefore IDEMPOTENT *side effects* in every handler (publication key per run, analytics per video+day, workflow step keys). Verified: re-executing a publish job does not publish twice. | P5 §5,§9, ADR-030 |
| ISS-P5-12 | No **engine-level observability** (correlation/run/tenant IDs, per-job metrics feeding P3/P2 surfaces) | Medium | M11/M8 | **Done (M11 Phase F)** — engine observability at `/admin/queue/jobs`: queue depth, running, dead-letter, active workers (live leases), per-job type/tenant/attempts/priority/lease/workflow-link/error; dead jobs escalate to `event_log`. | P5 §11 |
...
| ISS-P5-R1-06 | No **Worker Management Center** (active/idle/failed/restart/health/capacity/current+historical jobs/logs/perf; distributed-ready) | Medium | M11/M8 | **Done via the existing Queue/Job Manager (M11 Phase F)** — extended rather than duplicated: worker health (active lease holders), current/historical jobs, failure detail, and re-drive/cancel verbs. A separate Worker Center was deliberately NOT built. | P5R1 §17.6 |
...
| ISS-P5-R1-12 | No **Self-Healing Automation** (auto-retry/intelligent-delay/alt-provider/resource+queue+stuck-job recovery/deadlock-detect/auto-escalate before human) | High | M11 | **Partially done (M11 Phase D/E)** — automatic retry with exponential backoff, stuck-job recovery via lease reaping, alternative-provider failover + circuit breakers, and operator re-drive. Predictive/self-tuning recovery and deadlock detection remain future work. | P5R1 §17.12, ADR-039 |

## Part-6 additions (AI Generation Pipeline — `product-bible/PART-6-ai-generation-pipeline.md`, ADR-040…044)
| ID | Issue / gap | Sev | Task | Status | Source |
|---|---|---|---|---|---|
| ISS-P6-01 | No **full idea→publish content pipeline** (Strategy/Trend/Competitor/Fact-Verify/Story-Enhance/Compliance/Learning stages missing or mocked) | Critical | M12/M4 | **Structure done (M12 G6)** - expanded pipeline to 29 stages incl. Strategy/Trend/Competitor/Fact-Verify/Story-Enhance/Compliance(pre-render+pre-publish)/Quality-Gate/Learning, executed via the M11 DAG. Strategy, quality_gate and both compliance gates are REAL (rules/derived). Trend, competitor, fact_verify, story_enhance and learning are EXECUTION-GATED: explicitly skipped with the exact blocking dependency recorded - they never fabricate intelligence. | P6 §2,§3 |
| ISS-P6-02 | No **Content Quality Engine** (dimensioned explainable scores, partial-regeneration, threshold gating) | High | M12 | **Done (M12 G3)** - explainable, dimensioned, rules-based Quality Engine (script completeness, scene coverage, duration fit, SEO, brand alignment, continuity, safety) with per-tenant configurable weights/thresholds, narrowest-scope regeneration (partial then full), blocking dimensions forcing manual review; persisted to `quality_scores` with per-dimension evidence. An AI-evaluator tier is a later authorized addition (`evaluator` column distinguishes tiers). | P6 §5, ADR-042 |
| ISS-P6-03 | No **enterprise Prompt Engine** (templates/vars/versioning/governance/localization/optimization/testing/simulation/cost-estimate) | High | M12/M8 | **Done (M12 G1)** - prompt templates are first-class versioned assets in the unified Asset Library (template + variables in the versioned body, immutable approved versions, one active version, governance states). Optimization/simulation/cost-estimate remain future work. | P6 §6, ADR-041 |
| ISS-P6-04 | No **Character Consistency Engine** (reference-set identity, descriptors, voice binding, master-once, series continuity) | High | M12 | **Done (M12 G1)** - characters are versioned library assets with descriptors and `asset_bindings` (character to voice/style) for continuity; existing `characters` rows were backfilled into the governed library. Image-level identity consistency depends on paid generation (deferred). | P6 §7, ADR-041 |
| ISS-P6-05 | No **Visual Style Engine** (versioned reusable Style Packs, copy-on-use, format-independent, safety-aware) | Medium | M12 | **Done (M12 G1)** - style packs are versioned library assets with immutable versions and copy-on-use adoption. | P6 §8, ADR-041 |
| ISS-P6-06 | No **content-generation Decision Engine** (model-tiering, regeneration-scope, provider-switch, approval — explainable) | High | M12 | **Done (M12 G4)** - explainable decision records (`decision_records`: chosen, rejected alternatives, signals, rationale, cost estimate) written by the AI Gateway and by the quality/compliance gates. EXTENDS the existing gateway (ADR-037) - no second routing system was created. | P6 §9, ADR-037 |
| ISS-P6-07 | No **Content Memory** (tenant-isolated structured+semantic learning store; dedupe/reuse/steer) | High | M12 | **Structured tier done (M12 G4)** - tenant-isolated `content_memory` (normalized topic keys, usage counts, story links, similarity-based dedupe) read during generation and written after. Performance is linked ONLY from `source='live'` analytics - sample data is never learned from. Semantic/vector memory is deferred with R1-03 (pgvector deliberately not installed). | P6 §10, ADR-043 |
| ISS-P6-08 | No **Fact Verification + Compliance/Safety** stages (accuracy + policy/brand-safety gating) | High | M12 | **Compliance done, fact-verify gated (M12 G3/G6)** - rules-based Compliance/Safety engine runs as explicit pre-render and pre-publish gates (unsafe content, kids/news strictness, likeness consent, asset licensing, publish-metadata completeness, AI disclosure); blocking findings stop the run. Fact verification is execution-gated pending the Knowledge/RAG index (R1-03). | P6 §3.1,§2, ADR-044 |
| ISS-P6-09 | No **format-agnostic pipeline config** (one pipeline for Shorts/Long/Stories/future platforms) | Medium | M12 | **Done (M12 G2)** - `format_profiles` make format pure CONFIG (aspect, duration bounds, scene budget, pacing, caption/audio, publishing provider) consumed by the one pipeline; tenant profiles override platform defaults; generation resolves and applies the profile. | P6 §1, ADR-040 |
| ISS-P6-10 | No **pipeline-level cost optimization** (batch gen, duplicate detection, provider comparison, savings suggestions beyond caching) | Medium | M12/M6 | **Surfaced (M12 G5)** - provider cost comparison (cost/call), prompt-cache utilization, asset reuse and pHash duplicate detection in the Pipeline Analytics Center, built on existing `api_usage`/`prompt_cache`/`assets`. Batch generation remains future work. | P6 §14.9 |
| ISS-P6-11 | No **Human Review for generation** (inline edit/partial-regenerate/compare/approval-chains at pipeline gates) | Medium | M12 | **Integrated (M12 G3)** - quality/compliance gate outcomes drive the existing review loop (manual_review action, blocking gates) and are visible per run. Inline editing and side-by-side compare remain future work. | P6 §11 |
| ISS-P6-12 | No **localization/multi-language generation** (voice + subtitle variants) | Low | M12 | **Dimension done, execution gated (M12 G2)** - `locale` is a first-class column on stories/videos/plan_items and `content_variants` records locale-variant intent with its gate reason. Translation + localized voice execution is deferred (paid AI). | P6 §2,§3 |

## Part-6 Revision 1 additions (12 content-intelligence enhancements — `product-bible/PART-6-…` §16, ADR-045…049)
| ID | Issue / gap | Sev | Task | Status | Source |
|---|---|---|---|---|---|
| ISS-P6-R1-01 | No **Multi-Format Content Engine** (Format Profiles + repurpose one gen → many platform outputs; YT/Shorts/Reels/TikTok/LinkedIn/X/Pinterest/Podcast/future) | High | M12 | **Config foundation done (M12 G2)** - Format Profiles + `content_variants` provide the multi-format contract (one master to many format variants, each recording its gate reason). Repurposing RENDER execution is deferred (paid render). | P6R1 §16.1, ADR-045 |
| ISS-P6-R1-02 | No **Content Variation Engine** (multiple hooks/titles/thumbnails/CTAs/scripts/voices/scenes/endings; A/B) | Medium | M12/M8 | **Deferred - execution-gated.** Contract exists via `content_variants`; variation GENERATION requires authorized paid AI. No mock variations produced. | P6R1 §16.2 |
| ISS-P6-R1-03 | No **Knowledge Engine** (trusted sources/KB/client-docs/PDF/crawl/RAG/citations/fact-confidence/hallucination-detect/freshness; tenant-isolated) | High | M12 | **Deferred - execution-gated.** Requires paid embeddings + pgvector (deliberately NOT installed). The `fact_verify` stage exists and is gated with this dependency recorded. | P6R1 §16.3, ADR-046 |
| ISS-P6-R1-04 | No **Brand Voice Engine** (versioned tone/vocab/style/CTA/emotion/personality; multiple brand profiles) | Medium | M12 | **Done (M12 G1)** - brand voice is a versioned library asset kind (`brand_voice`) with immutable versions and active-version governance, supporting multiple profiles per workspace. | P6R1 §16.4, ADR-041 |
| ISS-P6-R1-05 | No **SEO Intelligence Engine** (intent/competitor-gap/trending/metadata/hashtags/tags/chapters/cards/end-screens/playlists) | Medium | M12 | **Deferred - execution-gated.** `trend`/`competitor` stages exist and are gated; they require live external trend/competitor data sources. No fabricated signals. | P6R1 §16.5 |
| ISS-P6-R1-06 | No **Thumbnail Intelligence** (scoring/CTR-prediction/variations/face-detect/text-opt/heatmap/brand-consistency) | Medium | M12 | **Deferred - execution-gated.** CTR prediction requires real CTR history (live YouTube Analytics) + image models. No predicted scores are shown. | P6R1 §16.6, ADR-018 |
| ISS-P6-R1-07 | No **Content Compliance Engine** (copyright/unsafe/policy/sensitive/AI-disclosure-future/brand-violation detection; explainable) | High | M12 | **Rules tier done (M12 G3)** - explainable compliance rule engine (unsafe content, audience strictness, likeness consent, asset licensing, publish metadata, AI disclosure) at both gates, persisted to `compliance_checks`. Copyright/AI classifier detection is a later authorized tier. | P6R1 §16.7, ADR-044 |
| ISS-P6-R1-08 | No **Learning Engine** (views/watch-time/CTR/likes/comments/retention/subs/geo/time/SEO-rank → improves future gens) | High | M12 | **Deferred - execution-gated.** The `learning` stage exists and is gated; it requires live YouTube Analytics (OAuth). Content Memory explicitly refuses to learn from sample data. | P6R1 §16.8, ADR-043 |
| ISS-P6-R1-09 | No **Multi-Language Engine** (translation/localized script/voice/thumbnail/SEO/regional refs; locale dimension, no rebuild) | Medium | M12 | **Deferred - execution-gated.** Locale dimension + variant contract are in place (G2); translation and localized voice require authorized paid AI. | P6R1 §16.9, ADR-047 |
| ISS-P6-R1-10 | No **Content Calendar Engine** (calendar/holidays/events/series/campaigns/weekly-themes/seasonal as generation input) | Medium | M12/M10 | **Done (M12 G2)** - the calendar is a real generation input: `plan_items` gained campaign/theme/locale/format_profile_id and generation consumes the item planned for the day (topic/angle) instead of inventing one (ADR-048). | P6R1 §16.10, ADR-048 |
| ISS-P6-R1-11 | No **unified versioned Asset Library** (characters/backgrounds/music/logos/intros/outros/transitions/voices/prompts/style-packs) | High | M12 | **Done (M12 G1)** - unified versioned Asset Library (`asset_library_items`/`asset_versions`/`asset_bindings`) covering prompt templates, characters, style packs, brand voices and voice profiles: immutable approved versions (DB trigger), exactly one active version, governance states, copy-on-use provenance, bindings. Legacy characters/voices backfilled. Surface at /library. | P6R1 §16.11, ADR-049 |
| ISS-P6-R1-12 | No **Pipeline Analytics Center** (per-stage success/failure/cost/time/quality/regen-count/review-count/provider-usage/opt-suggestions) | Medium | M12/M8 | **Done (M12 G5)** - Pipeline Analytics Center at /admin/pipeline: per-stage success/failure/skip/attempts/cost, provider comparison, cache + reuse/dedupe, quality & compliance gate outcomes. EXTENDS the M11 observability layer - no parallel analytics storage was created. | P6R1 §16.12 |

## Part-7 additions (Auth / AuthZ / Enterprise Security — `product-bible/PART-7-auth-security.md`, ADR-050…054)
| ID | Issue / gap | Sev | Task | Status | Source |
|---|---|---|---|---|---|
| ISS-P7-01 | No **unified identity model** (platform vs tenant planes + service accounts/API users; in-plane custom roles) | High | M13/M1 | **Done (M13 S2)** - formal identity planes enforced by constraint (platform vs tenant, never bridging); `service_accounts` as non-human identities; API identities remain M8 `api_keys`; custom roles are subsets WITHIN a plane. Operators still hold no tenant membership (ADR-002). | P7 §2, ADR-050 |
| ISS-P7-02 | No **MFA/SSO enforcement policy** + **enterprise SSO (SAML/OIDC) + SCIM** — TOTP exists but not policy-enforced | High | M13 | **Split: enforcement done (M13 S1), SSO/SCIM execution-gated.** MFA/AAL2 is now policy-enforced through the Security Policy Engine (required / required_for_roles / step_up_actions) and evaluated by the Zero-Trust engine. Real Supabase TOTP already existed. SAML/OIDC + SCIM require an external IdP and a Supabase SSO plan - seam documented, deliberately NOT faked. | P7 §3, ADR-053 |
| ISS-P7-03 | No **custom roles + permission groups + temporary permissions + approval-based escalation (PAM)** | High | M13 | **Done (M13 S2)** - `custom_roles`+`custom_role_permissions`, `permission_groups`+items, and `privileged_grants` (PAM) with DB-enforced separation of duties (a request can never be self-approved) and mandatory time-boxing; auto-expiry runs as the durable `security.pam_expire` job (M11). | P7 §4, ADR-051 |
| ISS-P7-04 | No **Organization tier + teams/departments + ownership transfer** (only single-tenant memberships) | High | M13/M1 | **Done (M13 S2) - STRICTLY ADDITIVE.** `organizations`/`organization_members` + nullable `tenants.organization_id`. A workspace with no organization behaves exactly as before (zero migration for existing customers). `my_tenant_ids()` is deliberately UNCHANGED, so organization membership grants NO implicit tenant access; org access is a separate explicit path (`my_org_ids()`). Verified live. | P7 §5, ADR-026 |
| ISS-P7-05 | No **enterprise session management** (active sessions/revoke/fingerprint/suspicious-activity/location-history/forced-logout) | Medium | M13 | **Done (M13 S3)** - `user_sessions` inventory with device fingerprint, IP, last-seen, per-session revoke (actor + reason) and a zero-trust `trust_level` supporting mid-session revocation. Global sign-out (pre-existing) retained. | P7 §6 |
| ISS-P7-06 | No **API security suite** (scoped keys, rotation, signed+replay-protected requests/webhooks, IP allowlists) | High | M13/M8 | **Done (M8 + M13 S1)** - M8 delivered scoped hashed keys, rotation, revoke, rate limiting and HMAC-signed outbound webhooks. M13 adds the remainder: key `expires_at` + max-age policy, CIDR-aware IP allowlists, and inbound request signature verification with timestamp replay protection - all enforced inside the existing `authenticateRequest` seam. | P7 §7 |
| ISS-P7-07 | No **full Vault lifecycle** (rotation/versioning/access-policies/health/expiry/usage-audit) — extends ISS-C3/M2 | High | M13/M2 | **Done (M13 S4)** - Vault lifecycle over the existing M3 seam: credential versioning (`credential_versions`), rotation metadata, expiry/health with the durable `security.vault_health` job, per-tenant/provider access policies, and grant/deny usage audit (`credential_access_log`). Secret VALUES still live only in Supabase Vault - no plaintext is stored. | P7 §8, ADR-054 |
| ISS-P7-08 | No **unified Security Center + risk score** (platform + tenant scoped) | Medium | M13/M8 | **Done (M13 S5)** - Security Center at /admin/security: posture, explainable risk score (every point attributed to a named contributor), open findings, incidents, credential health and audit-chain integrity. Subsumes the M8-deferred ISS-P2-09. | P7 §9 |
| ISS-P7-09 | No **immutable, hash-chained audit** across all security events (tamper-evidence) | High | M13 | **Done (M13 S5) - closes an ACTIVE gap.** `audit_log` previously carried a FOR ALL policy letting members UPDATE/DELETE their own audit rows; it is now INSERT+SELECT only with an UPDATE-blocking trigger. New `security_audit` is append-only and hash-chained (seq/prev_hash/hash built by a DB trigger, so a caller cannot forge them), with `verify_security_audit_chain()` for integrity proof and a sanctioned retention-purge path. | P7 §10, ADR-052 |
| ISS-P7-10 | No **compliance framework** (SOC2/ISO evidence, data residency/classification/retention, consent + sub-processor register) | Medium | M13/M8 | **Largely done (M13 S5)** - `data_classifications` (5 levels + handling + retention) with a platform baseline, `compliance_controls` evidence register (SOC2/GDPR/ISO with real evidence pointers), retention rules, and the Privacy Center. Sub-processor register and formal residency enforcement remain open. | P7 §11 |
| ISS-P7-11 | No **risk-based / step-up authentication** (anomalous-login detection) | Medium | M13 | **Done for real signals (M13 S1/S3)** - risk-based step-up is evaluated by the Zero-Trust engine (new device, new network, elevated/critical risk score, sensitive action). Impossible-travel is explicitly GATED: it requires an IP-geolocation provider and is declared in `GATED_DETECTORS` rather than inventing coordinates. | P7 §3,§6, ADR-053 |
| ISS-P7-12 | No **trusted devices + login notifications + device history** | Low | M13 | **Done (M13 S3)** - `trusted_devices` registry (fingerprint, first/last seen, explicit trust) + a `new_device` detector that raises an explainable finding for notification. | P7 §3 |

## Part-7 Revision 1 additions (10 Zero-Trust & security-ops enhancements — `product-bible/PART-7-…` §14, ADR-055…059)
| ID | Issue / gap | Sev | Task | Status | Source |
|---|---|---|---|---|---|
| ISS-P7-R1-01 | No **Zero Trust** enforcement (per-request re-auth/re-authz, continuous context-aware verification, mid-session trust revocation) | High | M13 | **Done (M13 S1)** - `evaluateZeroTrust` re-authorizes every request against identity, AAL, device/network familiarity, session trust and risk score, returning allow / step_up / deny with explicit reasons. Trust is revocable mid-session via `user_sessions.trust_level`. | P7R1 §14.1, ADR-055 |
| ISS-P7-R1-02 | No **central versioned Security Policy Engine** (password/MFA/session/login/IP/device/API/secret/data-access; tighten-only inheritance) | High | M13 | **Done (M13 S1)** - central versioned Security Policy Engine: one ACTIVE version per (scope, policy_type), immutable once activated (DB trigger), with TIGHTEN-ONLY inheritance platform -> organization -> tenant. `violatesTightenOnly` rejects any attempt to weaken an inherited policy. 5 platform baselines seeded. | P7R1 §14.2, ADR-056 |
| ISS-P7-R1-03 | No **Data Classification** (Public/Internal/Confidential/Restricted/Secret + handling + retention rules) | Medium | M13 | **Done (M13 S5)** - `data_classifications` with Public/Internal/Confidential/Restricted/Secret, handling rules and retention days; a platform baseline classifies the data this product actually holds. `isExportAllowed` enforces that secret-classified data can never leave. | P7R1 §14.3 |
| ISS-P7-R1-04 | No **DLP** (sensitive/secret/PII detection, export monitoring, download policies, watermarking-future) | Medium | M13 | **Done (M13 S5)** - deterministic DLP: secret-pattern detection (provider keys, platform keys, webhook secrets, JWTs, private keys) blocks exports; PII patterns trigger redaction. Evidence records match counts and a REDACTED sample - never the raw secret. Findings persist to `dlp_findings`. | P7R1 §14.4 |
| ISS-P7-R1-05 | No **Enterprise KMS** (platform-managed + BYOK-future keys; rotation/expiry/versioning/health/audit) | Medium | M13/M2 | **Platform-managed done; BYOK is a documented seam (M13 S4)** - `kms_keys` models the key hierarchy (root/data-encryption) with rotation/expiry/health/versioning. A BYOK row cannot be marked active without a real external key reference (DB CHECK), so an external KMS can never be faked as connected. | P7R1 §14.5, ADR-057 |
| ISS-P7-R1-06 | No **Security Incident Center** (detection/severity/response-workflow/timeline/evidence/resolution/post-incident report) | High | M13/M8 | **Done (M13 S3)** - `security_incidents` with severity, assignment, acknowledgement, resolution and an append-style timeline; threat findings link to incidents. Lifecycle verified live. | P7R1 §14.6 |
| ISS-P7-R1-07 | No **Threat Detection Engine** (brute-force/cred-stuffing/impossible-travel/API-abuse/token-abuse/priv-esc/secret-abuse/abnormal-automation; explainable) | High | M13 | **Done for available signals (M13 S3)** - explainable rules-based detectors over signals the platform already records: brute force, credential stuffing, API abuse, secret abuse, abnormal automation, new device. Each finding carries evidence + a recommended action and runs via the durable `security.threat_scan` job. Impossible-travel is explicitly gated (no geo data). | P7R1 §14.7, ADR-058 |
| ISS-P7-R1-08 | No **Security Analytics Center** (risk/login/threat trends, API abuse, session stats, Vault usage, audit growth, compliance status) | Medium | M13/M8 | **Done (M13 S5)** - security analytics derived from existing audit/session/API/job data on the Security Center (no duplicate analytics storage). | P7R1 §14.8 |
| ISS-P7-R1-09 | No **Privacy Center** (export/right-to-delete/consent/cookie-future/residency/privacy-requests/processing-history) | Medium | M13/M8 | **Done (M13 S5)** - Privacy Center: `privacy_requests` lifecycle (export/delete/rectify/consent-withdraw with statutory due dates) and `consent_records`. Existing export/deletion actions are retained and now tracked as requests. | P7R1 §14.9 |
| ISS-P7-R1-10 | No **Business Continuity** (security backups/DR/account-recovery/break-glass/emergency-admin/recovery-audit) | High | M13/M8 | **Done (M13 S6)** - sealed break-glass: multi-approval quorum (default 2) with DB-enforced separation of duties, mandatory future-dated time-box, auto-expiry via the durable `security.break_glass_expire` job, alarm/audit on every transition and a post-review field. Verified live: self-approval, sub-quorum activation, missing expiry and past expiry are all rejected. | P7R1 §14.10, ADR-059 |
